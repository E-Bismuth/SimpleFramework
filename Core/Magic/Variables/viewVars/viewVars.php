...
class viewVars implements magicVariablesInterface
{
    ...
    {
        if($force == false){
            throw new variablesException('You can\'t set/edit this field ('. $field .') on class viewVars');
        }
        else{
            $ArraySession = explode('/',$field);
            if(count($ArraySession)==1){
